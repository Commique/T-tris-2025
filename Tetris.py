...
while game_on:
    ...
    if running:
        ...
        #pygame.draw.rect(main_window, color[0], pygame.Rect(width/2 + 7*pixel, height/2, len(blocs[bloc_bundle[4][1][bloc_bundle[4][0]]][0])*pixel, len(blocs[bloc_bundle[4][1][bloc_bundle[4][0]]])*pixel))
        for y in range(len(blocs[bloc_bundle[4][1][bloc_bundle[4][0]]])):
            for x in range(len(blocs[bloc_bundle[4][1][bloc_bundle[4][0]]][y])):
                ...
                """elif blocs[bloc_bundle[4][1][bloc_bundle[4][0]]][y][x] == 0:
                    pygame.draw.rect(main_window, color[1], pygame.Rect((x+7)*pixel + width/2 + 1, y*pixel + height/2 + 1, pixel-2, pixel-2))"""
    ...
    score_displayRect.topleft = (width/2 + 7_*pixel, height/2 - 11*pixel)
